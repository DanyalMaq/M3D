...
from utils import crop_image_around_lesion, tsclasses, sentence_templates
...
def process_item(item, root_dir, output_dir):
    ...
    for key in ["pet", "ct"]:
        ...
        data = nii.get_fdata().transpose(2, 1, 0)[np.newaxis, ...][:,:,20:-20,20:-20]  # Shape: (1, z, y, x) . The y,x range is to remove air and scanner
        image_data[key] = data
    ...
    for seg in os.listdir(required_folders['seg']):
        ...
        data = nii.get_fdata().transpose(2, 1, 0)[np.newaxis, ...][:,:,20:-20,20:-20]  # Shape: (1, z, y, x)
        seg_data = data
        seg_data = np.clip(seg_data, 0, 1)

        # Crop CT and PET/mask around lesion
        ct_cropped, _ = crop_image_around_lesion(image_data["ct"], seg_data, 32, -300, 400)
        pet_cropped, mask_cropped = crop_image_around_lesion(image_data["pet"], seg_data, 32, 0, 12)

        # Now make the text
        label = tsclasses[number]
...
def create_dataset_parallel(root_dir, output_dir, max_items=-1):
    with concurrent.futures.ThreadPoolExecutor() as executor:
        # Submit each directory processing task
        futures = [executor.submit(process_item, item, root_dir, output_dir) 
                   for item in os.listdir(root_dir)[:max_items]]
